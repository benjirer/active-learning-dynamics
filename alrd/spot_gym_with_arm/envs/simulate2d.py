...
from alrd.spot_gym_with_arm.envs.spot2d import Spot2DReward, MIN_X, MIN_Y, MAX_X, MAX_Y
from alrd.utils.utils import Frame2D
import math
from alrd.spot_gym_with_arm.utils.utils import MAX_SPEED, MAX_ANGULAR_SPEED, get_hitbox
...
class Spot2DBaseSim(gym.Env, ABC):
    ...
    def __init__(
        self, config: SpotEnvironmentConfig, action_cost=0.0, velocity_cost=0.0
    ):
        self.config = config
    ...
    def reset(self, seed=None, options=None):
        ...
        pose = options.get(
            "pose", (self.config.start_x, self.config.start_y, self.config.start_angle)
        )
        goal = options.get(
...
class Spot2DEnvSim(Spot2DBaseSim):
    ...
    def _update_state(self, action):
        ...
        x = (
            ...
            + (self.state[3] - action_global[0]) * self.dt / 2
        )
        y = (
            self.state[1]
            + self.state[4] * self.dt
            + (self.state[4] - action_global[1]) * self.dt / 2
        )
        theta = (
            self.state[2]
            + self.state[5] * self.dt
            + (self.state[5] - action[2]) * self.dt / 2
        )
