...
class Trajectory2DWrapper(Wrapper):
    ...
    def __save_trajectory(self):
        ...
        position = rotate_2d_vector(
            trajectory[:, :2], goal_in_env[2], degrees=False
        ...
        plt.scatter(
            position[:, 0],
            position[:, 1],
            c=np.linspace(0.2, 1, position.shape[0]),
            cmap="Reds",
        )
        ...
        plt.arrow(
            0.0,
            0.0,
            0.5 * np.cos(goal_in_env[2]),
            0.5 * np.sin(goal_in_env[2]),
            color="green",
            width=0.05,
        )
        ...
        plt.plot(
            ...
            np.arctan2(trajectory[:, 3], trajectory[:, 2]),
        )
