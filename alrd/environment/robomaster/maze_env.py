...
from alrd.environment.robomaster.subscriber import (
    RobotSub,
    MazeMarginChecker,
    VelocityActionSub,
    ChassisSub,
...
def create_maze_env(
    coordinates, margin=0.22, freq=50, transforms=None, global_act=False
):
    _robot = init_robot()
    subscriber = ChassisSub(_robot.chassis, freq=freq)
    maze = Maze(coordinates, margin=margin)
    return MazeEnv(
        _robot, subscriber, maze, transforms=transforms, global_act=global_act
    )


def create_maze_goal_env(
    goal,
    coordinates=None,
    margin=0.22,
    freq=50,
...
class MazeEnv(BaseRobomasterEnv):
    def __init__(
        ...
        **kwargs
        ...
        self.maze = maze
        self._margin_checker = MazeMarginChecker(
            self.robot.chassis,
            self.subscriber.position_sub,
            self.subscriber.attribute_sub,
            self.__velocity_cmd_sub,
            maze,
            slide_wall=slide_wall,
            freq=subscriber.freq,
        )

    def _apply_action(self, action):
        action[:2] = rotate_2d_vector(action[:2], self._last_obs[2])
        action = self._margin_checker.check_action(
            *self._last_obs[:3], *action
        )  # TODO: may want to make the checker monitor speed asynchronously
        action[:2] = rotate_2d_vector(action[:2], -self._last_obs[2])
...
class MazeGoalVelocityEnv(MazeGoalEnv, VelocityControlEnv):
    ...
    @staticmethod
    def create_env(
        goal, freq=50, coordinates=None, margin=0.22, slide_wall=False, transforms=None
    ):
        return MazeGoalVelocityEnv._create_maze_goal_env(
            goal, freq, coordinates, margin, slide_wall, transforms
        )
...
class MazeGoalPositionEnv(MazeGoalEnv, PositionControlEnv):
    ...
    def create_env(
        ...
        return MazeGoalPositionEnv._create_maze_goal_env(
            goal,
            freq,
            coordinates,
            margin,
...
class MazeGoalKinemEnv(MazeGoalEnv, VelocityControlEnv, BaseEnvAccel):
    ...
    def create_env(
        ...
        return MazeGoalKinemEnv._create_maze_goal_env(
            goal, freq, coordinates, margin, slide_wall, transforms
        )
