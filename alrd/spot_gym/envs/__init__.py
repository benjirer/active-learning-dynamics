...
from alrd.spot_gym.model.robot_state import SpotState, KinematicState, modify_2d_state
...
# from opax.utils.model_checkpoint import predict_multistep


# def predict_states_2d_model(
#     states: List[SpotState],
#     commands: List[MobilityCommand],
...
def get_2d_buffer(session: Session) -> ReplayBuffer:
    buffer = ReplayBuffer(
        obs_shape=Spot2DEnv.obs_shape,
        action_shape=Spot2DEnv.action_shape,
        max_size=1000000,
    )
    for i in range(len(session)):
        episode = session.get_episode(i)
        x, y, _, qx, qy, qz, qw = episode.obs[
            0
        ].pose_of_body_in_vision  # TODO assumes it starts in the origin, could be different
        angle = R.from_quat([qx, qy, qz, qw]).as_euler("xyz", degrees=False)[2]
        goal_frame = Frame2D(x, y, angle)
        obs = np.array(
            [
                Spot2DEnv.get_obs_from_state_goal(state, goal_frame)
                for state in episode.obs
            ]
        )
        action = np.array(
            [Spot2DEnv.get_action_from_command(cmd) for cmd in episode.actions]
        )
        tran = Transition(
            obs=obs[:-1],
            action=action,
