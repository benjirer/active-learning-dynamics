...
class SpotEnvironmentConfig(yaml.YAMLObject):
    yaml_tag = "!SpotEnvironmentConfig"
    # robot hostname
    hostname: str
    ...
    def __post_init__(self):
        ...
        assert (
            self.start_x >= self.min_x and self.start_x <= self.max_x
        ), "start_x must be within the environment boundaries"
        assert (
            self.start_y >= self.min_y and self.start_y <= self.max_y
        ), "start_y must be within the environment boundaries"


##### Fixed environment parameters #####
MARGIN = 0.8  # Margin to the walls within which the robot is stopped (m)
CHECK_TIMEOUT = (
    MARGIN / BODY_MAX_VEL
...
class SpotBaseModel(object):
    ...
    def _shutdown(self) -> None:
        ...
        if (
            ...
            or not self.has_robot_control
            or not self.estop_keepalive
            or not self.robot.is_powered_on()
        ):
    ...
    def _toggle_estop(self) -> None:
        ...
        if not self.estop_keepalive:
            ...
            estop_endpoint = bosdyn.client.estop.EstopEndpoint(
                client=self.estop_client, name=self.client_name, estop_timeout=9.0
            )
    ...
    def _gain_control(self) -> None:
        ...
        try:
            ...
            print(
                "Another controller "
                + exc.response.lease_owner.client_name
    ...
    def _issue_robot_command(
        self, command: Command, endtime=None, blocking=False
    ) -> None:
    ...
    def _issue_goal_pose_command(self, x, y, theta, timeout=POSE_TIMEOUT) -> bool:
        ...
        while not done and endtime > current:
            try:
                cmd_id = self.command_client.robot_command_async(
                    cmd, end_time_secs=endtime
                ).result()
                done = block_for_trajectory_cmd(
                    self.command_client, cmd_id, timeout_sec=endtime - time.time()
                )
            except:
                if self.verbose == SpotVerbose.VERBOSE:
                    self.logger.warning(
...
class StateMachineRequest:
    ...
    def wait(self, timeout=None, sleep_period=0.1) -> Optional[bool]:
        ...
        while not finished and (timeout is None or time.time() - start < timeout):
            wait = (
                min(timeout - (time.time() - start), sleep_period)
                if timeout is not None
                else sleep_period
            )
            finished = self.__event.wait(timeout=wait)
...
class SpotBaseStateMachine(SpotBaseModel):
    ...
    def start(self):
        ...
        self.logger.info("Stand command issued")
        # determine body pose in vision frame, which is used to reset the robot
        self.set_start_frame()
        self.logger.info("Start frame set")
        self._set_reset_pose(
            self.config.start_x, self.config.start_y, self.config.start_angle
        )
    ...
    def set_start_frame(self):
        ...
        self.logger.info("State is:" + state.to_str())
    ...
    def __cmd_loop(self):
        try:
            ...
            while self.__state != State.SHUTDOWN:
                ...
                time.sleep(max(0.0, timelength - EXPECTED_STATE_READ_TIME))
                ...
                self.__queue.put(
                    StateMachineRequest(
                        StateMachineAction.STEP_DONE, (new_state, read_time, oob)
                    )
                )
                self.__cmd_event.wait()
        except Exception as e:
    ...
    def _issue_command(
        self, cmd: Command, timelength: float
    ) -> Tuple[bool, Optional[Tuple[Optional[SpotState], Optional[float], bool]]]:
        request = StateMachineRequest(StateMachineAction.STEP, (cmd, timelength))
        self.__queue.put(request)
        response = request.wait()
        return response

    def _issue_unmonitored_command(
        self, cmd: Command, timelength: float
    ) -> Tuple[bool, Optional[Tuple[Optional[SpotState], Optional[float], bool]]]:
        request = StateMachineRequest(
            StateMachineAction.UNMONITORED_STEP, (cmd, timelength)
        )
        self.__queue.put(request)
        response = request.wait()
        return response
    ...
    def __shutdown_loop(self):
        try:
            ...
            while self.__state != State.SHUTDOWN:
                ...
                self.__shutdown_event.clear()
                self.__queue.put(
                    StateMachineRequest(StateMachineAction.SHUTDOWN_DONE, True)
                )
                self.__shutdown_event.wait()
        except Exception as e:
    ...
    def is_in_bounds(self, state: SpotState) -> bool:
        ...
        max_x, max_y = np.max(box, axis=0)
        return (
            min_x > self.config.min_x + MARGIN
            and max_x < self.config.max_x - MARGIN
            and min_y > self.config.min_y + MARGIN
            and max_y < self.config.max_y - MARGIN
        )
        ...
        try:
            ...
            while self.__state not in {State.SHUTDOWN, State.SHUTTING_DOWN}:
                ...
                if new_state is None:
                    ...
                    self.__queue.put(
                        StateMachineRequest(StateMachineAction.CHECK_DONE, None)
                    )
                elif not self.is_in_bounds(new_state):
                    self.__queue.put(
                        StateMachineRequest(StateMachineAction.CHECK_DONE, False)
                    )
                    self.logger.info(
                        f"Robot out of bounds. box {self._get_spot_hitbox(new_state)}"
                    )
                else:
                    self.__queue.put(
                        StateMachineRequest(StateMachineAction.CHECK_DONE, True)
                    )
                self.__check_bounds.wait()
        except Exception as e:
    ...
    def __main_loop(self):
        ...
        try:
            while self.__state != State.SHUTDOWN:
                wait = MAX_MAIN_WAIT
                if (
                    self.state in {State.RUNNING, State.READY}
                    and monitor_on
                    and not check_ongoing
                ):
                ...
                if request.action == StateMachineAction.STEP:
                    ...
                    if self.__state == State.RUNNING:
                        ...
                        if new_state is None or (
                            oob and monitor_on
                        ):  # next state could not be read or out of bounds
                            if oob:
                                self.logger.info(
                                    f"Robot out of bounds after step. box {self._get_spot_hitbox(new_state)}"
                                )
                            else:
                                self.logger.info(
                                    "Robot state could not be read after step. Stopping."
                                )
                            monitor_on = False
                    ...
                    if self.__state in {State.RUNNING, State.READY} and monitor_on:
                        ...
                        if request.value is False:
                            ...
                            shouldstop = True
                            self.logger.info(
                                "Robot state could not be checked. Stopping."
                            )
                            if latest_order is not None:
                    ...
                    if self.__state == State.RESETTING:
                        if request.value:
                            if check_ongoing:
                                ...
                                self.logger.debug(
                                    f"State Machine: sending feedback to reset {request.value}"
                                )
                                latest_order.set_feedback((request.value, None))
                                latest_order = None
                                self.__state = State.READY
                            ...
                            self.__stop_event.set()
                    self.logger.debug(
                        f"State Machine: state after reset done {self.__state}"
                    )
                    ...
                    if self.__state in {State.READY, State.RUNNING}:
                        ...
                        )  # Note: we enter here only if reset is called after check_done triggered a stop
                        latest_order = request
                    else:
                        request.set_feedback((False, None))
